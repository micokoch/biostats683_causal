...
exer0717$PAQ655[exer0717$PAQ650 == 2] <- 0
exer0717$PAD660[exer0717$PAQ650 == 2] <- 0
summary(exer0717)
...
hist(exer0717$lg2vigexminwk) # Now we have a normally distributed set of values
summary(exer0717)
### Moderate exercise
...
exer0717$PAQ670[exer0717$PAQ665 == 2] <- 0
exer0717$PAD675[exer0717$PAQ665 == 2] <- 0
summary(exer0717)
# Make a simple imputation of the mean if days or minutes of exercise not specified
...
summary(exer0717)
hist(exer0717$targetex, breaks = 2)
# Create smaller dataframe with fewer columns
smex <- exer0717 %>% 
  select(SEQN, targetex, exminwk, modexminwk, vigexminwk, lg2exminwk, lg2modexminwk, lg2vigexminwk)
summary(smex)
...
summary(sleep0717)
hist(sleep0717$targetslp, breaks = 2)
hist(sleep0717$slphrs, breaks = 10)

##### Final Tables
# Merge sleep and exercise tables
slpex <- merge(smex, sleep0717, by = 'SEQN')
summary(slpex)
write_csv(slpex, "slpex.csv")
# Binary table
binslpex <- select(slpex, SEQN, targetex, targetslp)
summary(binslpex)
